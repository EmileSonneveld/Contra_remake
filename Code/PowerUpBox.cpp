...
PowerUpBox::~PowerUpBox() {
	// dont delete the static Bmp, windows will do it when closing the program
...
void PowerUpBox::CollideWith(ObjectBase *colliderptr) {
	if (colliderptr->GetType() == TYPE_PLAYER_BULLET) { // die and trow the powerup.
...
{
	m_AnimationTick += deltaTime * 8 * m_AnimationSense; // Progress tha animation a bit
